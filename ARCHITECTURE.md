...
### 1. Agents
- **Hermes (Brain):** The orchestrator. Resides in `~/.hermes/` and uses persistent memory to retain project context across sessions. It listens to Slack `#sprint-main` and uses the `status-report` skill to track OpenClaw's output. Model: `gemini-3.1-flash-lite` (via Gemini free tier).
- **OpenClaw (Hands):** The coding agent. Receives commands from Hermes via Slack `#agent-coder` and executes shell/code commands locally. Model: `qwen2.5-coder` (via local Ollama free tier).
...
4. **Report:** OpenClaw confirms execution via `status-report` skill.
5. **Verify:** Hermes updates the `#sprint-main` thread that the step is done.

## Security & Constraints
- **Zero Paid APIs:** Only 100% free models were used.
